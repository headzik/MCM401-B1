            ...
            while (r.next()) {
                ...
                switch (geom.getGeoType()) {
                    case Geometry.POINT:
                        ...
                        mObjects.add(new POIObject(id, IDrawingContext.POI_TYPE, new Polygon(new int[]{(int) p.getX()},new int[]{(int) p.getY()},1), loadImage("resources/gas.png"), POI_TYPE.GAS));
                        break;
                    default:
                        break;
                }
            ...
            while (r.next()) {
                ...
                switch (geom.getGeoType()) {
                    case Geometry.POINT:
                        String wkt = geom.toString();
                        org.postgis.Point p = new org.postgis.Point(wkt);
                        mObjects.add(new POIObject(id, IDrawingContext.POI_TYPE, new Polygon(new int[]{(int) p.getX()},new int[]{(int) p.getY()},1), loadImage("resources/police.png"), POI_TYPE.POLICE));
                        break;
                    default:
                        break;
                }
